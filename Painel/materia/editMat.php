...
if(!isset($_SESSION['user'])){
	...
	if(isset($_POST['acao']) && $_POST['acao'] == "editar"){
		...
		while($row = $consulta->fetch(PDO::FETCH_OBJ)){
		  ...
		  $nome = $_POST["nome".$id];
		  $desc = $_POST["desc".$id];
		  $muda = $conn->prepare('UPDATE materia SET nome=:nome, descricao=:d WHERE id = :id'); 
		  $muda->bindParam(':nome', $nome);
		  $muda->bindParam(':d', $desc);
		  $muda->bindParam(':id', $id);
		  $muda->execute();
		}
		header('location:home.php?msg=Materia(s) editada(s) com sucesso!');
	}else{
	include_once("conn.php");
	$consulta = $conn->prepare("SELECT * FROM materia");
	$consulta->execute();
						...
						<form action="" method="post" class="form-horizontal">
						  <fieldset>
							...
							<div class="control-group">
							  ...
							  <div class="controls">
								<input type="text" class="span6 typeahead" name="nome<?php echo $row->id; ?>" value="<?php echo $row->nome; ?>">
							  </div>
							</div>
							...
							<div class="control-group">
							  ...
							  <div class="controls">
								<input type="text" class="span6 typeahead" name="desc<?php echo $row->id; ?>" value="<?php echo $row->descricao; ?>" >
							  </div>
							</div>
                                     <?php } ?>                      
