...
if(!isset($_SESSION['user'])){
	...
	if(isset($_POST['action']) && $_POST['action'] == 'excluir'){
		...
		foreach ($ids as $id => $value) {
			...
			$stmt->execute();
			$sql = "DELETE FROM pergunta WHERE id_quiz =  :id";
			$peg = $conn->prepare($sql);
			$peg->bindParam(':id', $value, PDO::PARAM_INT);   
			$peg->execute();
			$sql = "DELETE FROM resposta WHERE id_quiz =  :id";
			$res = $conn->prepare($sql);
			$res->bindParam(':id', $value, PDO::PARAM_INT);   
			$res->execute();
		}
		header('location: home.php?msg=Quiz excluido com sucesso!');
	}else{
